$(function() {
	...
	getCurrentOrder(companyId, function(order) {
		if(order != "") {
			showCurrentOrder();
		} else {
			$(".no-order").show();
	...
	function showCurrentOrder() {
		$.get("http://localhost:8080/me/orders/coffees", function(userOrder) {
			var itemsHtml = "";
		...
		$.get("http://localhost:8080/me/orders/unorderedCoffees", function(coffees) {
			var itemsHtml = "";
	...
	$("#coffees").on("change", ".list-group-item", function() {
		...
		$.ajax("http://localhost:8080/me/orders/coffees", { 
	...
	$("#orderedCoffees").on("click", ".label-danger", function() {
		...
		$.ajax("http://localhost:8080/me/orders/coffees/" + coffeeId, {
			type : 'DELETE',
			success: function() {
	...
	function updateCartPrice() {
		$.get("http://localhost:8080/me/orders/price", function(price) {
	...
	$(".btn-init-order").click(function() {
		$.ajax({
			url: "http://localhost:8080/companies/" + companyId + "/orders",
			type: "PUT",
...
function getCurrentOrder(companyId, callback) {
	$.get("http://localhost:8080/companies/" + companyId + "/order", function(order) {
		callback(order);
	});
}
